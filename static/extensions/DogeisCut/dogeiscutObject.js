...
//      - Mostly works now, but the specific references between vars is not preserved. So if var a stored var b and vice versa, then you reloaded, var a would store a copy of var b that stores var a, and vice versa.
//          - idea to fix this is a compeltley custom seraliser that handles all the variables at once as an array or object of some sort, because the individual processing of all the variables is to blame here...
...
(function(Scratch) {
        ...
        for (let i = 0; i < raw.length; i++) {
            ...
            if (Array.isArray(item)) {
                for (let j = 0; j < item.length; j++) {
                    cache[i][j] = resolve(item[j]);
                }
            } else {
                for (const key in item) {
                    cache[i][key] = resolve(item[key]);
                }
            }
        }
    ...
    function flatten(root) {
        ...
        function helper(obj) {
            ...
            if (Array.isArray(obj)) {
                ...
                for (let item of obj) {
                    placeholder.push(helper(item));
                }
            } else {
                placeholder = {};
                objects.push(placeholder);
                for (const key in obj) {
                    placeholder[key] = helper(obj[key]);
                }
            }
    ...
    class Extension {
            ...
            vm.runtime.registerSerializer(
                ...
                v => {
                    if (v instanceof dogeiscutObject.Type) {
                        return {
                            entries: Object.entries(flatten(v.object)).map(([key, value]) => {
                ...
                v => {
                    try {
                        ...
                        return new dogeiscutObject.Type(inflate(Object.fromEntries(parsed)));
                    } catch {
                        return null;
                    }
                },
        ...
        getInfo() {
            return {
                ...
                blocks: [
                    ...
                    {
                        opcode: 'deepduplicate',
                        text: 'deep duplicate [OBJECT]',
                        arguments: {
                            OBJECT: dogeiscutObject.Argument,
                        },
                        ...dogeiscutObject.Block,
        ...
        duplicate({ OBJECT }) {
            ...
            return new dogeiscutObject.Type({ ...OBJECT.object });
        }

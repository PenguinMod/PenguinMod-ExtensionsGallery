...
class MIDI {
    ...
    getInfo() {
        return {
            ...
            blocks: [
                ...
                {
                    opcode: "notesOnChannel",
                    blockType: Scratch.BlockType.REPORTER,
                    text: "notes on channel [CHANNEL]",
                    arguments: {
    ...
    _onMIDISuccess(access) {
        ...
        // Just reuse the listener function instead of duplicating logic
        this.startListening();
    }
    ...
    _onMIDIMessage(event) {
        ...
        // MIDI constants (no magic numbers)
        ...
        if (isNoteOn) {
            if (isPad) {
                this._padOn(channel, note, velocity);
            } else {
                this._noteOn(channel, note, velocity);
            }
        }

        if (isNoteOff) {
            if (isPad) {
                this._padOff(channel, note);
            } else {
    ...
    _noteOff(channel, note) {
        if (this.activeNotes.has(channel)) {
            ...
            if (this.activeNotes.get(channel).size === 0) {
    ...
    _padOn(channel, note, velocity) {
        if (!this.activePads.has(channel)) {
            this.activePads.set(channel, new Map());
        }
    ...
    _padOff(channel, note) {
        if (this.activePads.has(channel)) {
            ...
            if (this.activePads.get(channel).size === 0) {
                this.activePads.delete(channel);
            }
        }
    ...
    setNote(args) {
        console.log(`Map note ${args.NOTE} → ${args.EVENT}`);
    }
    ...
    updateVisualizer() {
        ...
        for (const [ch, notes] of this.activeNotes.entries()) {
            for (const note of notes.keys()) {
                arr.push(`N Ch${ch}:${note}`);
            }
        }

        for (const [ch, pads] of this.activePads.entries()) {
            for (const pad of pads.keys()) {
                arr.push(`P Ch${ch}:${pad}`);
            }
        }
    ...
    notesPressed() {
        ...
        for (const pads of this.activePads.values()) {
            for (const pad of pads.keys()) {
                arr.push(pad);
            }
        }
    ...
    notesOnChannel(args) {
        ...
        if (this.activeNotes.has(channel)) {
            arr.push(...this.activeNotes.get(channel).keys());
        }

        if (this.activePads.has(channel)) {
            arr.push(...this.activePads.get(channel).keys());
        }
...
// ughhhhhh sightreadability... this aint a gd level :sob:

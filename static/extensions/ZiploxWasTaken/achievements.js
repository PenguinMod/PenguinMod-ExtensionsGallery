...
(function (Scratch) {
  ...
  const applyGlobalStyles = () => {
    // Inject custom root runtime targets variable arrays directly onto the container node
    const targets = [];
    const container = document.getElementById('AchievementEngineContainer');
    if (container) targets.push(container);
    
    // Also push variables down explicitly to active dynamically mounted child notification nodes

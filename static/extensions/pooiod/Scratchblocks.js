// Scratchblocks v1 (by pooiod7) - Make Scratch blocks in scratch
...
(function(Scratch) {
  ...
  function stageadd(elm) {
    const stage = canvas;
    stage.parentNode.appendChild(elm);
  }
  ...
  class scratchblocksext {
        ...
        blocks: [
          {
            ...
            text: 'Make stack [blocks] with id [id] and size [size]',
            arguments: {
              blocks: {
    ...
    makestack(args) {
      ...
      this.removestack({id:args.id})
      
      var blocks = document.createElement("p");
      blocks.textContent = args.blocks.replace(/\\n/g, "\n");
      blocks.classList.add(args.style);
      blocks.id = args.id;
      blocks.style.position = 'absolute';
      blocks.style.transform = 'translate(-50%, -50%)';
      
      addelm(blocks);

      var pageWidth = canvas.scrollWidth;
      var pageHeight = canvas.scrollHeight;
      var pageMagnitude = Math.sqrt(pageWidth * pageHeight)/800;
  
      scratchblocks.init();
      if (this.style == "sb2") {
        scratchblocks.module.renderMatching("#"+args.id, {
          style: "scratch2",
          scale: (args.size+1)*pageMagnitude,
        });
      } else if (this.style == "sb3") {
        scratchblocks.module.renderMatching("#"+args.id, {
          style: "scratch3",
          scale: args.size*pageMagnitude,
        });
      } else {
        scratchblocks.module.renderMatching("#"+args.id, {
      ...
      this.movestack({id:args.id,x:0,y:0})
    }
    ...
    removestack(args) {
      var element = document.querySelector('#page #'+args.id);
      if (element) {
    ...
    stackexists(args) {
      return !!document.querySelector('#page #'+args.id);
    }
    ...
    movestack(args) {
      ...
      if (element) {
        this.updatesizevars();
        element.style.marginTop = ((-(args.y+40)/this.stageheight+0.5)*100) + '%';
        element.style.marginLeft = ((args.x/this.stagewidth+0.5)*100) + '%';
      }
    }

(function(Scratch) {
  ...
  class HTMLtoCanvas {
        ...
        if (Object.keys(this.pages).includes(args.PAGE)){
            if(args.ID !== ""){
                if(!this.pages[args.PAGE].ids.includes(args.ID)){
                    ...
                    this.pages[args.PAGE].code = `${this.pages[args.PAGE].code}<!-- begin the ${args.ID} --><${args.EL} id="element${args.PAGE}${args.ID}"><!-- end the top --><!-- end the bottom of ${args.ID} -->`
                } else {
                    throw new Error (`Only one element with the id "${args.ID}" can exist in the document`)
                }
            } else {
                this.pages[args.PAGE].code = `${this.pages[args.PAGE].code}<!-- begin the ${args.ID} --><${args.EL} ><!-- end the top --><!-- end the bottom of ${args.ID} -->`
            }
        }
    }
    ...
    nestEl(args, util){
        if (Object.keys(this.pages).includes(args.PAGE)){
            if (util.stackFrame.startedBranch) {
                this.pages[args.PAGE].code = `${this.pages[args.PAGE].code}</${args.EL}><!-- end the bottom of ${args.ID} -->`
            ...
            if(args.ID !== ""){
                if(!this.pages[args.PAGE].ids.includes(args.ID)){
                this.pages[args.PAGE].ids.push(args.ID)
                this.pages[args.PAGE].code = `${this.pages[args.PAGE].code}<!-- begin the ${args.ID} --><${args.EL} id="element${args.PAGE}${args.ID}"><!-- end the top -->`
                } else {
                throw new Error (`Ony one element with the id "${args.ID}" can exist in the document`)
                }
            } else {
                this.pages[args.PAGE].code = `${this.pages[args.PAGE].code}<!-- begin the ${args.ID} --><${args.EL} ><!-- end the top -->`
            }
    ...
    text(args, util){
        if (Object.keys(this.pages).includes(args.PAGE)){   
            this.pages[args.PAGE].code = `${this.pages[args.PAGE].code}${args.TEXT}`
        }
    }
    ...
    movePage(args, util){
        if(Object.keys(this.pages).includes(args.PAGE)){
            ...
            console
            if(this.viewing === args.PAGE && document.querySelector("#htmlpage")){ 
                const elements = document.querySelectorAll(`#htmlpage`);
                elements.forEach(el => el.remove());
                const el = document.createElement("div");
                el.innerHTML = this.pages[args.PAGE].code;
    ...
    getHTML(args, util){
        ...
        return(`<html><body>${cleanString}</body></html>`)
        } else {
            return('Page does not exist!')
        }
    }
